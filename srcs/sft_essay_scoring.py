...
# trait_map = {
#     1: ["sentence_fluency", "word_choice", "conventions", "organization", "content"],
#     2: ["sentence_fluency", "word_choice", "conventions", "organization", "content"],
#     3: ["narrativity", "language", "prompt_adherence", "content"],
#     4: ["narrativity", "language", "prompt_adherence", "content"],
#     5: ["narrativity", "language", "prompt_adherence", "content"],
#     6: ["narrativity", "language", "prompt_adherence", "content"],
#     7: ["style", "conventions", "organization", "content"],
#     8: ["voice", "sentence_fluency", "word_choice", "conventions", "organization", "content"]
#     }
...
def prepare_dataset(examples, tokenizer, args, test=False):
    ...
    Score the essay in JSON format, using the trait names as keys, without any additional text.
    """
    ...
    if not test:
        ...
        for essay, prompt_id, output_text in zip(examples["essay"], examples["essay_set"], examples["score_labels"]):
...
def llama_test(model, tokenizer, test_dataset, args):
    ...
    qwk_result = dict()
    trait_map = {
    1: ["content", "organization", "word choice", "sentence fluency", "conventions"],
    2: ["content", "organization", "word choice", "sentence fluency", "conventions"],
    ...
    7: ["content", "organization", "style", "conventions"],
    8: ["content", "organization", "voice", "word choice", "sentence fluency", "conventions"]
    }
...
def main(args):
    ...
    model_saving_dir = os.path.join(model_saving_dir, "score_only")
    # model_saving_dir = os.path.join(model_saving_dir, f"{args.dimension}_with_score_reverse")
    if not os.path.exists(model_saving_dir):
        os.makedirs(model_saving_dir)
    for fold in range(5):
        ...
        args.checkpoint_dir = os.path.join(args.checkpoint_dir, "score_only")
