...
class PreferenceDataLoader:
    def __init__(self, data_path: str, tokenizer: AutoTokenizer, test: bool):
        """
    ...
    def _format_conversation(self, essay, rationale) -> list:
        """Format a conversation with dialog history and response."""
        conversation = []

        # Add system prompt
        system_prompt = {"role": "system",
                         "content": "Judge the specificity of the teacher’s evaluation of the given essay. Focus on how specific the evaluation is, whether it includes direct citations from the essay, and the breadth of those citations."}
...
def evaluate_preference_accuracy(
    ...
    data_loader = PreferenceDataLoader(data_path, model.tokenizer, test=True)
...
if __name__ == "__main__":
    ...
    data_path = "../data/speceval_dataset.json"
    save_path = "./llama_3b_specificity_reward_model"
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    
    
    model = RewardModel(MODEL_NAME)
    data_loader = PreferenceDataLoader(data_path, model.tokenizer, test=False)
    ...
    for epoch in range(args.epochs):
        for batch in tqdm(data_loader.get_evaluation_pairs(batch_size), desc="Evaluating", total=int(len(data_loader.dataset)/ batch_size)):
            chosen_scores = model.get_scores(batch['chosen'], batch_size=batch_size)
            rejected_scores = model.get_scores(batch['rejected'], batch_size=batch_size)
            true_scores = torch.tensor(batch['true_score']).to(chosen_scores.device)
